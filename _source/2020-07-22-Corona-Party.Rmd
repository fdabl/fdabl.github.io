...
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, message = FALSE, fig.align = 'center', fig.width = 10, fig.height = 5)
```
...
# From https://dadescovid.cat/?drop_es_residencia=2&id_html=ambit_1&tipus=regio&codi=13
...
# See also https://www.nytimes.com/interactive/2020/world/europe/italy-coronavirus-cases.html
italy_dat <- covid19('Italy', level = 3, verbose = FALSE) %>% 
  filter(
    # From 22nd July to 4th August
...
# See also https://www.nytimes.com/interactive/2020/world/europe/germany-coronavirus-cases.html
germany_dat <- covid19('Germany', level = 2, verbose = FALSE) %>% 
  filter(
    # From 22nd July to 4th August
    date >= '2020-07-21', date <= '2020-08-04'
  )
...
```{r risk plot sensitivity, echo = FALSE, dpi = 400, fig.height = 9, fig.width = 12}
...
plot_total_risk <- function(party_sizes, true_relative_cases, ...) {
  ...
  plot(
    true_relative_cases, ns, type = 'n', xaxs = 'i', yaxs = 'i', axes = FALSE,# log = 'x',
    xlab = 'Estimated True Number of Infectious Cases per 100,000 Inhabitants', ylab = 'Party Size', ...
  )
...
(london_risk <- party_risk(c(10, 25, 50), 5 * london_cases) * 100)
(amsterdam_risk <- party_risk(c(10, 25, 50), 5 * amsterdam_cases) * 100)
(barcelona_risk <- party_risk(c(10, 25, 50), 5 * barcelona_cases) * 100)
```
...
```{r, eval = FALSE, echo = FALSE}
(risks <- party_risk(105, 5 * c(london_cases, amsterdam_cases, barcelona_cases)) * 100)
```
...
```{r, eval = FALSE, echo = FALSE}
(risks <- party_risk(30, 1 * c(london_cases, amsterdam_cases, barcelona_cases)) * 100)
(risks <- party_risk(30, 5 * c(london_cases, amsterdam_cases, barcelona_cases)) * 100)
(risks <- party_risk(30, 10 * c(london_cases, amsterdam_cases, barcelona_cases)) * 100)
```
...
```{r, echo = TRUE, eval = FALSE, dpi = 400, fig.height = 8}
library('RColorBrewer')
