...
```{r, echo = FALSE, warning = FALSE, fig.align = 'center', fig.width = 12, fig.height = 6, dpi=400}
library('latex2exp')
...
t <- seq(0, 5, 0.01)
fn <- function(t, r = 1, N0 = 2) N0*exp(r*t)
N <- seq(0, 100, .01)
...
plot(
  ...
  ylab = '', lwd = 2,
  xlab = 'N', main = TeX('$$\\dot{N}$$ = N$$'),
  cex.main = cm, cex.lab = cl, font.main = 1, xaxs = 'i', yaxs = 'i'
)
mtext(TeX('$$\\dot{N}$$'), side = 2, line = 2.75, cex = cl)
axis(1, cex.axis = ca)
axis(2, las = 2, cex.axis = ca)

plot(
  t, fn(t), type = 'l', axes = FALSE, lwd = 2,
  ylab = 'N(t)', xlab = 'Time t',  main = TeX('$$N(t) = N_0 e^{r t}$$'),# main = 'Exponential Growth',
  cex.main = cm, cex.lab = cl, font.main = 1, ylim = c(0, 300), xaxs = 'i', yaxs = 'i'
)
axis(1, cex.axis = ca)
axis(2, las = 2, cex.axis = ca)
```
...
N <- seq(0, 1, .01)
t <- seq(0, 10, .01)
logistic <- function(N, r = 1, K = 1) { r*N * (1 - N/K) }
sigmoid <- function(t, r = 1, K = 1, N0 = 0.1) { K / (1 + (K - N0) / N0 * exp(-r*t))}

par(mfrow = c(1, 2))

plot(
  N, logistic(N), type = 'l', axes = FALSE,
  ylab = '', lwd = 2,
  xlab = 'N', main = TeX('$$\\dot{N} = N(1 - N)$$'),
  cex.main = cm, cex.lab = cl, font.main = 1, xaxs = 'i', yaxs = 'i'
)
mtext(TeX('$$\\dot{N}$$'), side = 2, line = 2.75, cex = cl)
axis(1, cex.axis = ca)
...
plot(
  t, sigmoid(t), type = 'l', axes = FALSE, lwd = 2,
  ylab = 'N(t)', xlab = 'Time t', main = TeX('$$N(t) = \\frac{1}{1 + \\frac{1 - N_0}{N_0} e^{-rt}}$$'),# main = 'Sigmoidal Growth',
  cex.main = cm, cex.lab = cl, font.main = 1, ylim = c(0, 1), xaxs = 'i', yaxs = 'i'
)
axis(1, cex.axis = ca)
axis(2, las = 2, cex.axis = ca)
```
...
cols <- brewer.pal('Set1', n = 5)

solve_logisticRK <- function(N0, r = 1, delta_t = 0.01, times = 1000) {
  N <- rep(N0, times)
  dN <- function(N) r * N * (1 - N)
...
logistic <- function(time, N0 = 0.10, r = 1) 1 / (1 + (1 - N0) / N0 * exp(-r*time))

add_line <- function(delta, col, N0 = 0.10, fn = solve_logistic) {
  time <- seq(0, 10, length.out = 10 / delta)
  lines(
    time,
    fn(N0, times = 10 / delta, delta = delta),
    col = scales::alpha(col, 0.75), lwd = 2
  )
}

add_lineRK <- function(delta, col, N0 = 0.10, fn = solve_logisticRK) {
  add_line(delta, col, N0, solve_logisticRK)
}

get_error <- function(delta, N0 = 0.10) {
  time <- seq(0, 10, length.out = 1000)
  true <- logistic(time, N0)
  approx <- solve_logistic(N0, times = 10 / delta, delta = delta)
  abs(true[1000] - approx[10 / delta])
}


plot_approximation <- function(fn = add_line, main = 'Euler\'s Method') {
  time <- seq(0, 10, length.out = 1000)
  plot(
    time, logistic(time, 0.10), type = 'l', lwd = 2, axes = FALSE,
    main = main, ylab = 'N(t)', xlab = 'Time t',
    cex.main = cm, cex.lab = cl, font.main = 1, ylim = c(0, 1), lty = 2, xaxs = 'i', yaxs = 'i'
  )
  axis(1, cex.axis = ca)
  axis(2, las = 2, cex.axis = ca)
  
  deltas <- c(5, 2, 1, 0.1, 0.01)
  fn(5, cols[1])
  fn(2, cols[2])
  fn(1, cols[3])
  fn(0.1, cols[4])
  fn(0.01, cols[5])
  lines(time, logistic(time, 0.10), lty = 2, lwd = 2)
  
  legend(
    'topleft', lty = c(2, rep(1, 5)), lwd = 2, col = c( 'black', scales::alpha(cols, 0.75)), bty = 'n',
    legend = c(
      'Analytic',
...
par(mfrow = c(1, 2))
plot_approximation(add_line)
# plot_approximation(add_line2, 'Runge-Kutta')

deltas <- 10^(-seq(1, 5, .1))
# deltas <- 10^(-seq(0, 4))
ldeltas <- log(deltas)
error <- Vectorize(get_error)(deltas)
lerror <- log(error)
plot(
  ldeltas, lerror,
  pch = 20, lwd = 2, axes = FALSE,
  main = 'Euler\'s Error', ylab = 'Absolute Error (Log)', xlab = expression('Log ' ~ Delta ~ 't'),
  cex.main = cm, cex.lab = cl, font.main = 1, ylim = c(-20, -8), xlim = c(-12, -2), xaxs = 'i', yaxs = 'i'
)
axis(1, cex.axis = ca)
axis(2, las = 2, cex.axis = ca)
```
...
```{r, echo = FALSE, warning = FALSE, fig.align = 'center', fig.width = 12, fig.height = 4, dpi=400}
par(mfrow = c(1, 3))
plot_SIR(solve_SIR(0.95, 0.05, beta = 1/8, gamma = 1/8, times = 4000), main = expression(beta ~ ' = 1/8,' ~ gamma ~ ' = 1/8'))
plot_SIR(solve_SIR(0.95, 0.05, beta = 3/8, gamma = 1/8, times = 4000), main = expression(beta ~ ' = 3/8,' ~ gamma ~ ' = 1/8'))
plot_SIR(solve_SIR(0.95, 0.05, beta = 6/8, gamma = 1/8, times = 4000), main = expression(beta ~ ' = 6/8,' ~ gamma ~ ' = 1/8'))
```
...
```{r, echo = FALSE, warning = FALSE, fig.align = 'center', fig.width = 12, fig.height = 4, dpi=400}
par(mfrow = c(1, 3))
plot_SIR(solve_SIR(0.95, 0.05, beta = 1/8, gamma = 1/12, times = 4000), main = expression(beta ~ ' = 1/8,' ~ gamma ~ ' = 1/12'))
plot_SIR(solve_SIR(0.95, 0.05, beta = 3/8, gamma = 1/12, times = 4000), main = expression(beta ~ ' = 3/8,' ~ gamma ~ ' = 1/12'))
plot_SIR(solve_SIR(0.95, 0.05, beta = 6/8, gamma = 1/12, times = 4000), main = expression(beta ~ ' = 6/8,' ~ gamma ~ ' = 1/12'))
```
...
plot_trajectory_SIR <- function(S0, I0, beta = 1, gamma = 1) {
  lines(solve_SIR(S0, I0, beta, gamma), col = 'red', lwd = 2)
}

par(mfrow = c(1, 2), mar = c(0, 0, 0, 0))
b <- 1/8
g <- 1/8
plot_vectorfield_SIR(
  beta = b, gamma = g,
  main = expression(beta ~ ' = 1/8,' ~ gamma ~ ' = 1/8')
)
plot_trajectory_SIR(0.8, 0.2, beta = b, gamma = g)
plot_trajectory_SIR(0.6, 0.4, beta = b, gamma = g)
plot_trajectory_SIR(0.4, 0.6, beta = b, gamma = g)
plot_trajectory_SIR(0.2, 0.8, beta = b, gamma = g)

b <- 3/8
g <- 1/8
plot_vectorfield_SIR(
  beta = b, gamma = g,
  main = expression(beta ~ ' = 3/8,' ~ gamma ~ ' = 1/8')
)
plot_trajectory_SIR(0.95, 0.01, beta = b, gamma = g)
plot_trajectory_SIR(0.8, 0.01, beta = b, gamma = g)
plot_trajectory_SIR(0.8, 0.2, beta = b, gamma = g)
plot_trajectory_SIR(0.6, 0.01, beta = b, gamma = g)
lines(c(g/b, g/b), c(0, 1), lty = 2, lwd = 1)
# plot_trajectory_SIR(0.6, 0.4, beta = b, gamma = g)
# plot_trajectory_SIR(0.4, 0.6, beta = b, gamma = g)
# plot_trajectory_SIR(0.2, 0.8, beta = b, gamma = g)
```
...
The figure below shows trajectories for a fixed recovery rate of $\gamma = 1/8$, a fixed reinfection rate of $\mu = 1/8$, and an increasing rate of infection $\beta$ for the initial condition $S_0 = 0.95$, $I_0 = 0.05$, and $R_0 = 0$. We take a time step $\Delta t = 1$ to denote one day.

```{r, echo = FALSE, warning = FALSE, fig.align = 'center', fig.width = 12, fig.height = 4, dpi=400}
par(mfrow = c(1, 3))
plot_SIRS(
  solve_SIRS(0.95, 0.05, beta = 1/8, gamma = 1/8, mu = 1/8, times = 4000),
  main = expression(beta ~ ' = 1/8,' ~ gamma ~ ' = 1/8' ~ mu ~ ' = 1/8')
)
plot_SIRS(
  solve_SIRS(0.95, 0.05, beta = 3/8, gamma = 1/8, mu = 1/8, times = 4000),
  main = expression(beta ~ ' = 3/8,' ~ gamma ~ ' = 1/8' ~ mu ~ ' = 1/8')
)
plot_SIRS(
  solve_SIRS(0.95, 0.05, beta = 6/8, gamma = 1/8, mu = 1/8, times = 4000),
  main = expression(beta ~ ' = 6/8,' ~ gamma ~ ' = 1/8' ~ mu ~ ' = 1/8')
)
```
...
par(mfrow = c(1, 3))
plot_SIRS(
  solve_SIRS(0.95, 0.05, beta = 1/8, gamma = 1/8, mu = 1/12, times = 4000),
  main = expression(beta ~ ' = 1/8,' ~ gamma ~ ' = 1/8' ~ mu ~ ' = 1/12')
)
plot_SIRS(
  solve_SIRS(0.95, 0.05, beta = 3/8, gamma = 1/8, mu = 1/12, times = 4000),
  main = expression(beta ~ ' = 3/8,' ~ gamma ~ ' = 1/8' ~ mu ~ ' = 1/12')
)
plot_SIRS(
  solve_SIRS(0.95, 0.05, beta = 6/8, gamma = 1/8, mu = 1/12, times = 4000),
  main = expression(beta ~ ' = 6/8,' ~ gamma ~ ' = 1/8' ~ mu ~ ' = 1/12')
)
```
...
```{r, echo = FALSE, warning = FALSE, fig.align = 'center', fig.width = 15, fig.height = 8, dpi=400}
plot_trajectory_SIRS <- function(S0, I0, beta = 1, gamma = 1, mu = 1/8, times = 4000) {
  lines(solve_SIRS(S0, I0, beta, gamma, mu, times = times), col = 'red', lwd = 2)
}
...
g <- 1/8
m <- 1/8
plot_vectorfield_SIRS(
  beta = b, gamma = g, mu = m,
  main = expression(beta ~ ' = 1/8,' ~ gamma ~ ' = 1/8,' ~ mu ~ ' = 1/8')
)
#plot_trajectory_SIRS(0.99, 0.01, beta = b, gamma = g, mu = m)
plot_trajectory_SIRS(0.1, 0.01, beta = b, gamma = g, mu = m, times = 50000)
plot_trajectory_SIRS(0.1, 0.9, beta = b, gamma = g, mu = m, times = 50000)
plot_trajectory_SIRS(0.1, 0.2, beta = b, gamma = g, mu = m, times = 50000)
lines(c(g/b, g/b), c(0, 1), lty = 2, lwd = 1)
lines(S, m * (1 - S) / (b*S + m), lty = 2)
...
plot_trajectory_SIRS(0.1, 0.00, beta = b, gamma = g, mu = m)
plot_trajectory_SIRS(0.95, 0.05, beta = b, gamma = g, mu = m)
plot_trajectory_SIRS(0.1, 0.01, beta = b, gamma = g, mu = m, times = 50000)
plot_trajectory_SIRS(0.1, 0.9, beta = b, gamma = g, mu = m)
plot_trajectory_SIRS(0.1, 0.2, beta = b, gamma = g, mu = m)
lines(c(g/b, g/b), c(0, 1), lty = 2, lwd = 1)
lines(S, m * (1 - S) / (b*S + m), lty = 2)
```
...
```{r, echo = FALSE, warning = FALSE, fig.align = 'center', fig.width = 12, fig.height = 6, dpi=400}
fn <- function(beta = 1/8, gamma = 1/8, mu = 1/8) mu * (1 - gamma / beta) / (gamma + mu)
beta <- seq(1, 200) / 8

par(mfrow = c(1, 2))
plot(
  (1/8)/beta, fn(beta = beta), type = 'l', axes = FALSE,
  ylab = '', xlab = expression(gamma / beta),
  main = 'Bifurcation Diagram for I', ylim = c(0, 0.5), xlim = c(0, 1),
  cex.main = cm, cex.lab = cl, font.main = 1, lwd = 2
)
mtext(expression(I^"*"), side = 2, line = 2.5, cex = cl)
lines(c(0, 1), c(0, 0), lwd = 2)
points(x = seq(0, 1, .05), y = rep(0, 21), cex = 1.4)
points(x = seq(0, 1, .05), y = rep(0, 21), cex = 1.5, pch = 20, col = 'white')
points(x = seq(0, 1, .05), y = seq(0.5, 0, length.out = 21), pch = 20, cex = 1.5)
arrows(.1, .05, .1, .3, length = .1, lwd = 2)
arrows(.4, .05, .4, .2, length = .1, lwd = 2)
arrows(.7, .05, .7, .1, length = .1, lwd = 2)
axis(1)
axis(2, las = 2)

plot(
  (1/8)/beta, (1/8)/beta, type = 'l', axes = FALSE,
  ylab = '', xlab = expression(gamma / beta),
  main = 'Bifurcation Diagram for S',
  cex.main = cm, cex.lab = cl, font.main = 1, lwd = 2
)
mtext(expression(S^"*"), side = 2, line = 2.5, cex = cl)
lines(c(0, 1), c(1, 1), lwd = 2)
points(x = seq(0, 1, .05), y = rep(1, 21), cex = 1.4)
points(x = seq(0, 1, .05), y = rep(1, 21), cex = 1.5, pch = 20, col = 'white')
points(x = seq(0, 1, .05), y = seq(0, 1, .05), pch = 20, cex = 1.5)
arrows(.1, .9, .1, .4, length = .1, lwd = 2)
arrows(.4, .9, .4, .6, length = .1, lwd = 2)
arrows(.7, .9, .7, .8, length = .1, lwd = 2)
axis(1)
axis(2, las = 2)
```
