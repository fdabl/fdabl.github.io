...
Bayesian inference using Markov chain Monte Carlo methods can be notoriously slow. In this blog post, we reframe Bayesian inference as an optimization problem using variational inference, markedly speeding up computation. We derive the variational objective function, implement coordinate ascent mean-field variational inference for a simple linear regression example in R, and compare the results to a Stan implementation. Sounds like word salad? Then let's start unpacking!
...
```{r, echo = FALSE, fig.width = 14, fig.height = 6, fig.align = 'center', message = FALSE, warning = FALSE, dpi=400, eval = FALSE}
par(mfrow = c(1, 2))

beta <- seq(-0.30, .9, .001)
plot(
  beta, dnorm(beta, mc$beta_mu, mc$beta_sd[length(mc$beta_sd)]),
  axes = FALSE, type = 'l', xlab = expression(beta), ylab = 'Density',
  ylim = c(0, 4), main = expression('Posterior of ' ~ beta),
  cex.main = 2, cex.lab = 1.5, cex.axis = 2
)

axis(1, at = seq(-0.50, 1, 0.2))
axis(2, las = 2) 

sigma2 <- seq(.4, 1.6, .001)
plot(
  sigma2, dinvgamma(sigma2, (100 + 1)/2, mc$nu[length(mc$nu)]),
  axes = FALSE, type = 'l', xlab = expression(sigma^2), ylab = 'Density',
  ylim = c(0, 4), main = expression('Posterior of ' ~ sigma^2),
  cex.main = 2, cex.lab = 1.5, cex.axis = 2
)
...
Whenever one goes down a rabbit hole of calculations, it is good to sanity check one's results. Here, we use Stan's variational inference scheme to check whether our results are comparable. They use mean-field variational inference as well, but they do it in a "black-box" way so that no problem-specific calculations are required (see Kucukelbir, Ranganath, Gelman, & Blei, 2015). Subsequently, we compare our results to the exact posteriors arrived by Markov chain Monte carlo. The simple linear regression model in Stan is:
...
model <- readRDS('../stan-compiled/variational-regression.RDS')
stan_dat <- list('n' = nrow(dat), 'x' = dat$x, 'y' = dat$y, 'tau' = 0.50)
fit <- vb(
  model, data = stan_dat, output_samples = 20000, adapt_iter = 10000,
  init = list('b' = 0.30, 'sigma' = 1), refresh = FALSE, seed = 1
...
Their recommendation is prudent. If you run the code with different seeds, you will notice that there is quite some variance in the posterior mean estimate; it can range from $0.12$ to $0.45$ --- and this even though Stan says that the ELBO has converged! So I would not trust these results.
...
We subsequently implemented the coordinate ascent mean-field variational inference (CAVI) algorithm in R, and compared our results to Stan's variational inference scheme as well as Stan's exact Markov chain Monte Carlo procedure. We have found that we probably should not trust Stan's variational inference implementation, and that our results roughly correspond to the exact procedure. For more on variational inference, I recommend the excellent review article by Blei, Kucukelbir, and McAuliffe ([2017](https://www.tandfonline.com/doi/full/10.1080/01621459.2017.1285773)).
...
---
## Footnotes
[^1]: The first part of this blog post draws heavily on the excellent review article by Blei, Kucukelbier, and McAuliffe (2017), and so I use their (machine learning) notation. I suggest you read their excellent paper after reading this blogpost!