...
```{r, fig.cap='*Figure 1*. Trying to fit lines with only one point (underdetermined).', echo = FALSE, fig.width = 5, fig.height = 5, fig.align = 'left'}
library('ggplot2')
ptheme <- theme(
...
# ggplot(dat[1, ], aes(x = x, y = y)) +
#   geom_point() +
#   geom_abline(intercept = b0(-.5), slope = -.5, col = 'skyblue') +
#   geom_abline(intercept = b0(0), slope = 0, col = 'skyblue') +
#   geom_abline(intercept = b0(1), slope = 1, col = 'skyblue') +
#   ylim(c(0, 4)) +
#   xlim(c(0, 4)) +
#   ggtitle('Underdetermined') +
#   ptheme
```
</div>
...
```{r, fig.cap = '*Figure 2*. Trying to fit lines with two points (determined).', echo = FALSE, fig.width = 5, fig.height = 5, fig.align = 'right'}
plot(1, type = "n", xlim = lim, ylim = lim,
...
```{r, fig.cap = '*Figure 3*. Trying to fit lines with three points (overdetermined).', echo = FALSE, fig.width = 5, fig.height = 5, fig.align = 'right'}
plot(1, type = "n", xlim = lim, ylim = lim,
...
#   geom_point() +
#   geom_abline(intercept = 2, slope = 0, col = 'skyblue') +
#   geom_abline(intercept = 4, slope = -1, col = 'skyblue') +
#   geom_abline(intercept = 2.5, slope = -.5, col = 'skyblue') +
#   ylim(c(0, 4)) +
#   xlim(c(0, 4)) +
#   ggtitle('Overdetermined') +
#   ptheme
```
</div>
...
```{r, fig.cap = '*Figure 1*. Trying to fit lines with only one point (top left; underdetermined), two points (top right; identified), three points (bottom left; overdetermined). Using Laplace\'s and Gauss\'s insights, we fit the  line which minimizes the absolute squared error (bottom right); horizontal lines show the errors.', echo = FALSE, fig.width = 8, fig.height = 6, eval = FALSE}
par(mfrow = c(2, 2))
...
pred <- predict(m)
abline(m, col = 'skyblue')
points(P1[1], P1[2], pch = 20)
points(P2[1], P2[2], pch = 20)
points(P3[1], P3[2], pch = 20)
segments(P1[1], P1[2], P1[1], pred[1])
segments(P2[1], P2[2], P2[1], pred[2])
segments(P3[1], P3[2], P3[1], pred[3])
mtext('Just fitting some lines ...', side = 3, line = -2, outer = TRUE)
```
...
```{r, fig.cap = '*Figure 3*. Trying to fit lines with three points (overdetermined).', echo = FALSE, fig.width = 5, fig.height = 5, fig.align = 'right'}
y <- c(2, 1, 2)
X <- cbind(c(1, 1, 1), c(1, 3, 2), c(1, 9, 4))
b <- solve(X) %*% y

x <- seq(0, 4, .01)
x <- cbind(1, x, x^2)
plot(1, type = "n", xlim = c(0, 4), ylim = c(0, 3),
     bty = "n", xlab = "x", ylab = "y", main = 'Curve fitting', yaxt = 'n'
)
axis(2, at = c(0, 1, 2, 3), las = 2)

lines(x[, 2], x %*% b, col = 'skyblue')
points(P1[1], P1[2], pch = 20)
points(P2[1], P2[2], pch = 20)
points(P3[1], P3[2], pch = 20)
...
# ggplot(dat, aes(x = x, y = y)) +
#   geom_point() +
#   geom_line(data = data.frame(x = x[, 2], y = x %*% b), col = 'skyblue') +
#   ylim(c(0, 3)) +
#   xlim(c(0, 4)) +
#   ggtitle('Curve fitting') +
#   ptheme
```
</div>
...
```{r, fig.cap = '*Figure 3*. Trying to fit lines with three points (overdetermined).', echo = FALSE, fig.width = 5, fig.height = 5, fig.align = 'right', eval = TRUE}
plot(1, type = "n", xlim = lim, ylim = lim,
     bty = "n", xlab = "x", ylab = "y", main = '"Best" fit', yaxt = 'n'
)
axis(2, las = 2)

d <- rev(dat)
abline(a = 2 + 2/3, b = -.5, col = 'skyblue')
points(d[1, ], pch = 20)
points(d[2, ], pch = 20)
points(d[3, ], pch = 20)
```
...
<div style= "float: left; padding: 10px 10px 10px 0px;">
```{r, fig.cap = '*Figure 3*. Trying to fit lines with three points (overdetermined).', echo = FALSE, fig.width = 5, fig.height = 5, fig.align = 'right', eval = TRUE}
plot(1, type = "n", xlim = lim - 2, ylim = lim - 2,
     bty = "n", xlab = "x", ylab = "y", main = 'Geometric aid', yaxt = 'n'
)
axis(2, las = 2)

d <- rev(data.frame(apply(dat, 2, function(x) x - mean(x))))

abline(a = 0, b = -.5, col = 'skyblue')

points(d[1, ], pch = 20)
points(d[2, ], pch = 20)
points(d[3, ], pch = 20)
segments(x0 = d$x[1], x1 = d$x[1], y0 = d$y[1], y1 = d$x[1]*-.5)
segments(x0 = d$x[2], x1 = d$x[2], y0 = d$y[2], y1 = d$x[2]*-.5)
segments(x0 = d$x[3], x1 = d$x[3], y0 = d$y[3], y1 = d$x[3]*-.5)
segments(x0 = d$x[3], x1 = d$x[3], y0 = d$y[3], y1 = -4, lty = 2)
```
</div>
...
<!-- <div style= "float: left; padding: 10px 10px 10px 0px;"> -->
```{r, fig.cap = '*Figure 2*. Illustrates the difference between the Laplace and the Gauss distribution.', echo = FALSE, fig.width = 7, fig.height = 6}
# library('rmutil')
# library('ggplot2')
# library('latex2exp')
# 
...
#   # stat_function(fun = function(x) dlaplace(x, 0, 1), aes(col = 'Laplace')) +
#   stat_function(fun = function(x) dnorm(x, 0, .5), aes(linetype = '1')) +
#   stat_function(fun = function(x) dnorm(x, 0, 1), aes(linetype = '2')) +
#   stat_function(fun = function(x) dnorm(x, 0, 1.5), aes(linetype = '3')) +
#   scale_linetype_manual('',
#                         values = c('solid', 'dashed', 'dotted'),
#                         labels = unname(TeX(c('$\\sigma^2 = 0.5$', '$\\sigma^2 = 1.0$', '$\\sigma^2 = 1.5$')))
#   ) +
#   ggtitle('Gaussian Error Curve') +
#   ylab('Density') +
#   xlab(TeX('$\\epsilon$')) +
#   theme(
#     plot.title = element_text(hjust = .5),
...
plot(1, type = "n", xlim = c(-4, 4), ylim = c(0, 1),
     bty = "n", ylab = "y", yaxt = 'n', xlab = expression(epsilon)
)
...
x <- seq(-4, 4, .01)
lines(x, dnorm(x, 0, .5), lty = 1)
lines(x, dnorm(x, 0, 1), lty = 2)
lines(x, dnorm(x, 0, 1.5), lty = 3)
title(ylab = 'y', las = 1)
title(main = 'Gaussian Error Curve')
legend(
  'topright', lty = c(1, 2, 3), box.lty = 0,
  legend = unname(TeX(c('$\\sigma^2 = 0.5$', '$\\sigma^2 = 1.0$', '$\\sigma^2 = 1.5$')))
)
```
<!-- </div> -->
...
```{r, echo = FALSE, fig.width = 9, fig.height = 7, fig.align = 'center', eval = FALSE}
set.seed(1776)

ns <- c(10, 50, 200, 500, 1000)

plot_clt <- function(rdist, ns, times = 500, ylab = 'Fequency', ...) {
  x <- replicate(times, mean(rdist(ns)))
  
  hist(
    x, breaks = 30, main = '',
    xlab = TeX('$$n_{e}$$'), col = 'grey76', ylab = '', yaxt = 'n'
  )
  
  axis(2, las = 2, ylab = ylab)
  
}


d <- data.frame(
  Beta = replicate(nt, mean(rbeta(ns, 1, 1))),
...
<div style= "float: left; padding: 10px 10px 10px 0px;">
```{r, fig.cap = '*Figure 2*. Illustrates that linear regression assumes that the conditional distribution of the response y given the features x is a Gaussian distribution with mean given by $\\mathbf{x}^T\\mathbf{b}$ and error variance $\\sigma^2$. Thus, the smaller the error variance, the better our predictions.', echo = FALSE, fig.width = 6, fig.height = 5}

set.seed(1776)
n <- 50
x <- rnorm(n)
y <- 2.5 - .5*x + rnorm(n, 0, .5)

plot(1, type = "n", xlim = c(-3, 3), ylim = c(0, 5),
     bty = "n", xlab = "x", ylab = "y", yaxt = 'n', cex = 1.5,
...
m <- lm(y ~ x)
abline(m, col = 'skyblue', lwd = 1.5)
points(x, y, pch = 20)

pred <- coef(m)[1]
sderr <- sd(resid(m))

add_normal <- function(x) {
  pred <- as.numeric(coef(m) %*% c(1, x))
  
  nn <- 1000
  xx <- seq(pred - 1.5, pred + 1.5, length.out = nn)
  
  for (i in seq(nn - 1)) {
    segments(x + dnorm(xx[i], pred, sderr), xx[i], x + dnorm(xx[i+1], pred, sderr), xx[i+1])
  }
  
  segments(x, pred, x + dnorm(pred, pred, sderr), pred, lty = 'dotted')
}
...
check <- function(y, x) {
    X <- cbind(1, poly(x, degree = length(y) - 1, raw = TRUE))
    b <- solve(X) %*% y
    cbind(y, X %*% b) # unnecessary of course
}
```
</div>
