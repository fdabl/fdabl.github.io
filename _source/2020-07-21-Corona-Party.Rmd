...
# July 21 first using https://coronadashboard.rijksoverheid.nl/regio?regio=VR13
# Different from https://www.rivm.nl/en/novel-coronavirus-covid-19/current-information
2.2 + 2.7 + 3.1 + 0 + 1.6 + 1.9 + 0.5 + 0.6 + 1.1 + 0.4 + 0.5 + 0.9 + 0.3 + 0.7
```
...
```{r risk plots, echo = FALSE, dpi = 400}
...
plot_total_risk <- function(party_sizes, true_relative_cases, ...) {
  ...
  plot(
    ...
    xlab = 'True Number of Infectious Cases per 100,000 Inhabitants', ylab = 'Party Size', ...
  )
...
```{r, echo = TRUE, eval = FALSE, dpi = 400}
plot_risk <- function(ns, true_relative_cases = 20.4 * 5, ...) {
  plot(
    ns, 1 - prob_virus_free(ns, true_relative_cases = true_relative_cases),
    type = 'l', axes = FALSE,
