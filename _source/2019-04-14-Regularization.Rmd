...
plot_likelihood <- function(y, n, ylab = 'Likelihood') {
  mle <- y/n
  x <- seq(.000, 1, .001)
  dens <- dbinom(y, n, x)
  max_dens <- max(dens)
  
  plot(x, dens, xlim = c(0, 1), type = 'l', ylab = ylab, lty = 1,
       xlab = TeX('$\\theta$'), las = 1, main = TeX(sprintf("$y = %d,  n = %d$", y, n)), lwd = 3,
       cex.lab = 1.5, cex.main = 1.5, col = 'skyblue', axes = FALSE
  )
  
  lines(c(mle, mle), c(0, max_dens), lty = 3)
  points(c(mle, mle), c(max_dens, max_dens), pch = 20)
...
plot_beta <- function(a, b, ylim, yticks, ylab = 'Density', title = NULL) {
  
  if (is.null(title)) {
    title <- TeX(sprintf("$a = %d,  b = %d$", a, b))
  }
  
  x <- seq(.000, 1, .001)
  dens <- dbeta(x, a, b)
  plot(x, dens, xlim = c(0, 1), type = 'l', ylab = ylab, lty = 1,
       xlab = TeX('$\\theta$'), las = 1, main = title, lwd = 3, ylim = ylim,
       cex.lab = 1.5, cex.main = 1.5, col = 'skyblue', axes = FALSE
  )
  
  axis(1, at = seq(0, 1, .2)) #adds custom x axis
  axis(2, las = 1, at = yticks) # custom y axis
}

par(mfrow = c(2, 3))
plot_beta(1, 1, ylim = c(0, 2), yticks = seq(0, 2, .5))
plot_beta(2, 1, ylim = c(0, 2), yticks = seq(0, 2, .5))
plot_beta(1, 2, ylim = c(0, 2), yticks = seq(0, 2, .5))
plot_beta(2, 2, ylim = c(0, 2), yticks = seq(0, 2, .5))
plot_beta(10, 10, ylim = c(0, 4), yticks = seq(0, 4, 1))
plot_beta(0.5, 0.5, title = TeX(sprintf("$a = %.1f,  b = %.1f$", .5, .5)), ylim = c(0, 4), yticks = seq(0, 4, 1))
```
...
```{r, echo = FALSE, fig.width = 14, fig.height = 7, fig.align = 'center', message = FALSE, warning = FALSE, dpi=400}
plot_path <- function(y, n, reg, title, ylab = '') {
  path <- estimate_path(y, n, reg)
  
  plot(path, ylim = c(0, 1), type = 'l', lty = 1,
       ylab = ylab, xlab = TeX('$\\lambda$'), main = title, lwd = 3,
       cex.lab = 1.5, cex.main = 1.5, col = 'skyblue', axes = FALSE)
  
  lines(c(0, 10), c(y/n, y/n), lty = 2)
  
  axis(1, at = seq(0, 10, 1))
  axis(2, las = 1, at = seq(0, 1, .2)) # custom y axis
}

par(mfrow = c(1, 2))
plot_path(3, 3, 1, 'L1-Norm Regularization Path', ylab = TeX('$\\theta$'))
plot_path(3, 3, 2, 'L2-Norm Regularization Path')
```
...
```{r, echo = FALSE, fig.width = 14, fig.height = 7, fig.align = 'center', message = FALSE, warning = FALSE, dpi=400}
plot_pen_llh <- function(y, n, lambdas, reg = 1, ylab = 'Penalized Likelihood', title = '') {
...
plot_pen_llh(3, 3, lambdas, reg = 2, title = TeX('$L_2$ Penalized Likelihood'), ylab = '')
```
